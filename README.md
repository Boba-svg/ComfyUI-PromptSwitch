...
##【PromptSwitch v0.4.4 リリースノート】  

■ 新機能  
・ワークフロー読み込み時にいずれかのノードタイトルに /Compact（大文字小文字問わず）があると、  
　全てのPromptSwitchノード（/v タグ付きを除く）が自動でコンパクトモードになる  
　→ Shift+V と同じ挙動です  
  
■ 既知の不具合
・ワークフロー保存の問題は解消されました以下の不具合を確認しています
・LORAマネージャーのアイコンが上部バーで [L][L] と二重表示になる場合があります  
　※作者環境では再現しており、原因は自動コンパクト処理の二重実行によるものです  
　※機能的には全く問題なく、見た目の問題のみです  
　※修正は難しそう
  
v0.4.3 まではこのバグは一切ありませんでした。  
「見た目が気になる」方は v0.4.3 を引き続きご利用ください。  
「自動コンパクトが欲しい」方は v0.4.4 をご利用ください。  
  
どちらも動作は完全に正常です。ご都合に合わせてお選びください！  
