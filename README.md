...
特にテキストエディター機能を持つことによるプロンプト編集の強力な応用性。
...
---
## ⚡️ \[Original] プロンプト管理をテキストエディターで出来る事による強力な応用性の一部をご紹介

* ** 他の方の作ったをプロンプトの導入のしやすさ **
    * プロンプトをコピー、別途エディターで”カンマ”を”カンマ＋改行文字”に置換して本ノードに貼り付けると即すべての要素がリスト化される

* ** プロンプト紹介サイト＋チャットAIを活用して労せずにリストを入手、再検索の手間を省く**
  　* "プロンプト 視点"などでgoogle検索をして、プロンプトを紹介しているサイトをみてプロンプトをみつけて、ComfyUIでプロンプトエディターで貼り付け、これやっていました私も
  　* 自分で編集してPromptSwitchノードで打ち込むの良いのですが、今回紹介する方法もためしていただけると能率がかなり上がると思います。
  　* チャットAIの入力画面にまずこううちます
bash
   こちらの中のプロンプトとそれに対するコメントを プロンプト // コメント という形で抽出してほしい、カテゴリが変わったら空白行を入れて 一回のコピーボタンですむようにまとめて”

   * するとチャットAIはプロンプト // コメントの形でそのページのプロンプトをリスト化してくれる上にコピーボタンまで用意してくれるので
  　* それをPromptSwitchに貼ってあげれば即強力なプロンプトリストを入手でき、忘れるたびにプロンプト紹介サイトを検索する手間から逃れることができます。   
