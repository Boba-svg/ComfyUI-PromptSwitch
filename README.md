...
## 💡 次の更新予定
2.  空白行のみの場合は改行文字もプロンプトに入れない　
3.  プロンプトの繋ぎ、およびプロンプトの最後に改行を入れないモードを用意する　⇒　出力プロンプトを綺麗にしたい人むけ　（自分は今の方が視認性がよくて好き）
4.  シフト＋Eで全てのノードをエディットモードで開くを導入　⇒　ブラウザの検索機能に引っかかるようになる
5.  ランダムピックアップの導入　⇒　そもそも技術的に可能なのか検討、一つだけ選ぶ、複数個チェックありの完全ランダム、区切り部分で囲まれた中からランダムピックなど
***
