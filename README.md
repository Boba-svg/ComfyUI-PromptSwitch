...
| **R** | **ランダムピックアップ** ✨ | 選択中のノード内のプロンプトを、**空行で区切られたセクション**から**それぞれ1つだけ**ランダムで有効化します。|
| **Shift + R** | **全ノードランダムピックアップ** 🎲 | **キャンバス上の全PromptSwitchノード**に対して、上記のランダムピックアップを一括実行します。**【除外タグ: `/r`, `/arv`, `/var`】**|
...
> * **`/av`** や **`/var`** など、複数のタグを組み合わせて指定することも可能です。
