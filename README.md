...
※画像は古い可能性あり。最新のショートカットは下記参照

**ノードタイトルのタグの書式は/区切りです**  
例：`/a /r /R0-4 /C`（`/ra` といった複合は無効）
...
### `/C` タグの特徴（最新仕様）
- **表示は常に `/C` のみ**（`/CM1-1` などに変わらない）
- **毎回確実にランダム実行**（1回限りではなく永遠に）
- **Cキー** で簡単にON/OFF
- **Shift+C** で全ノード一括解除
- **/Rタグと完全連携**（個数・外れ枠も自由）
...
- `/CM5` → 最大5回までランダム実行 → 5回目でリセット
- `/CM10-3` → 現在3回目
- 生成時に自動で `/CM10-4` → … → `/CM10-1` に更新
- **`/C` とは完全に独立**（併用不可、`/C` が優先）

---
## `/T` タグ（Turn: 順番に回る）【25/11/14実装】
...
### 2. 手動インストール
ComfyUI/custom_nodes フォルダで以下を実行
...
| 2025/11/14   | `/T` タグ実装 |
| **2025/11/15** | **`/C` 完全決定版**<br>・生成後も `/C` のまま残る<br>・毎回確実にランダム実行<br>・Cキーだけで完結<br>・`/CMn-k` と完全分離 |
