...
## 主な機能
※こちらの画像の説明は古い内容です。ショートカットキーなどは変更されております。（Rキーなど）
...
1. **PromptSwitchのノードのタイトルに/Rと/C を末尾を追加する **（/CタグはＣを押下すれば追加される）
（例：`顔の表情 /R0-3 /C`）  

2. **「実行（ＲＵＮ）」ボタンを押す**

→ **押すたびに、0〜3個の要素が自動でランダム選択され、画像が生成されます。**（ランダムピックアップの内容は/Rタグでコントロール）  
R キーなんて押す必要はありません。
