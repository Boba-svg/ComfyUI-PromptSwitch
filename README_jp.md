...
## 最強の新機能：/C タグ（生成ボタン1回ごとにプロンプトが自動ランダム更新）

> **「生成ボタンを押すたびに、プロンプトが自動で変わる」**

これこそが **PromptSwitch の真価**です。
...
| 従来の方法 | /C タグあり |
|------------|-------------|
| 毎回手動で R キー押す | **生成ボタン1回で自動更新** |
| 同じプロンプトが続く | **1回1回違う絵が出る** |
| 試行錯誤が遅い | **目的の絵に最短で到達** |
...
| /R-[m] | 1 〜 m 個 |
| /R[m]-[n] | n 〜 m 個（順不同） |
| **/C** | **生成ボタン1回ごとに自動ランダム更新** |

> **例：`髪型 /R2 /C`**  
> → 毎回 **2つの髪型** をランダム選択し、生成ボタンで即反映
