...
ComfyUI Managerが利用できない場合や手動での管理を希望される場合は、以下の手順でインストールしてください。
1.  ComfyUIがインストールされているフォルダ内の **\`custom_nodes\` フォルダ**へ移動します。
2.  cd /path/to/ComfyUI/custom_nodes
3.  git clone https://github.com/Boba-svg/ComfyUI-PromptSwitch.git
4.  ComfyUIを再起動すれば完了です！
...
3.  cd /path/to/ComfyUI/custom_nodes/ComfyUI-PromptSwitch
4.  git pull
