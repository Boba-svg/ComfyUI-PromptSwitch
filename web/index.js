 ...
 */
function randomPickupPrompts(text) {
    const lines = text.split('\n');
    const commentPrefix = "// ";
    const prefixRegex = /^\s*\/\/\s*/;
...
        const originalLineIndices = []; // コメント行やDisabled Phrase行も含めた元のインデックス

        for (let i = 0; i < section.length; i++) {
            const line = section[i];
            if (line.trimStart().match(/^\s*\/\/\s*disabled phrase\s*\d{14}$/)) {
                originalLineIndices.push({ type: 'disabled_phrase', index: i });
            } else if (line.trim() !== '') {
                validPromptIndices.push(i);
                originalLineIndices.push({ type: 'prompt', index: i });
            } else {
                originalLineIndices.push({ type: 'empty', index: i });
            }
        }

        // 3. ランダムで有効にする行を選択
        let randomIndex = -1;
        if (validPromptIndices.length > 0) {
            const randomLocalIndex = Math.floor(Math.random() * validPromptIndices.length);
            randomIndex = validPromptIndices[randomLocalIndex];
        }
...
                            return !isMuted && !isNodeExcluded(n, ['r']);
                        });

                        for (const node of targetNodes) {
                            const w = findTextWidget(node);
                            if (w) {
                                w.value = randomPickupPrompts(w.value);
...
                        textWidget.value = randomPickupPrompts(textWidget.value);
                        actionTaken = true;
                    }
                }
                
